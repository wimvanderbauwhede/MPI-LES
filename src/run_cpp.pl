...
#            say 'PUSH:'.$line,"\t",$placeholder;#Dumper(@blocks_stack);
            if (exists $macros_to_skip{$macro}) { 
                ...
                push @{ $stash_ref->{$srcfile}{$placeholder}}, $line;
                              say "PUSH LINE $line ON $placeholder (4)";

            } else {
            if ($in_block ne '') {
                push @{ $stash_ref->{$srcfile}{$placeholder}}, $line;
                              say "PUSH LINE $line ON $placeholder (3)";
            } else {
                # do nothing
                push @out_lines, $line;
            }
...
#            say 'POP:'.$macro,"\t",scalar @blocks_stack;
            # if this line is the end of a block, resume normal operation
            if (scalar @blocks_stack == 0 and $in_block eq $macro and $macro ne '') {
#                say $placeholder;
#                say "PUSH $line";
#                die if $placeholder == 7188 and $line =~/\#endif/;
                    say "PUSH LINE $line ON $placeholder (1) FOR $macro";
                push @{ $stash_ref->{$srcfile}{$placeholder}}, $line;
                $in_block='';
                $placeholder++;
                say "LINE: TAG $placeholder";
            } else {
                if ($in_block ne '') {
                    say "PUSH LINE $line ON $placeholder";
                    push @{ $stash_ref->{$srcfile}{$placeholder}}, $line;
                } else {
                    # do nothing
                    push @out_lines, $line;
            ...
            # if we're in a block, stash
             push @{ $stash_ref->{$srcfile}{$placeholder}}, $line;
              say "PUSH LINE $line ON $placeholder (2)";
            } else {
