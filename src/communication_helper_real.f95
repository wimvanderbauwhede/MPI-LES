...
! N = inNestedGrid, O = .not. inNestedGrid
! (N&N or O&O) or (N&O &0 ) or (O&N & 0)
! (NOT N&O) OR (N&O AND 0)
        nestToOrig =inNestedGrid() .and. .not. inNestedGridByRank(commWith)
        if ( ( nestToOrig .and. (syncTicks == 0) ) .or. .not. nestToOrig) then
#endif
