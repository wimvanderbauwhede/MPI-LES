...
! remainingDecls
      ...
      real(4) :: f2z !!
        ...
        integer, dimension(1) :: dmask1_ptr_sz
        integer, dimension(1) :: alpha_ptr_sz
        integer, dimension(1) :: dt_ptr_sz
        integer, dimension(3) :: beta_sz
        integer, dimension(3) :: jp_sz
        integer, dimension(3) :: ip_sz
        integer, dimension(3) :: f_sz
        integer, dimension(3) :: fx_sz
        integer, dimension(3) :: g_sz
        real(kind=4), dimension(0:(ip + 1),0:(jp + 1),0:(kp + 1),1) :: dmask1_ptr
        real(kind=4), dimension(1) :: alpha_ptr
        real(kind=4), dimension(1) :: dt_ptr
        real(kind=4), dimension(0:ip,0:jp,0:kp,1) :: fy_ptr
        real(kind=4), dimension(0:ip,0:jp,0:kp,1) :: h_ptr
        real(kind=4), dimension(0:ip,0:jp,0:kp,1) :: fz_ptr
...
! Size assignments
        ...
        wsum_sz = shape(wsum)
        w_sz = shape(w)
        dmask1_ptr_sz = shape(dmask1_ptr)
        ...
        call oclLoadBuffer(KP_BUF_IDX, kp_buf)
        ...
        call oclLoadBuffer(JP_BUF_IDX, jp_buf)
        call oclLoadBuffer(IP_BUF_IDX, ip_buf)
...
! ---- BEGIN feedbf_map_35 ----------------------------------------------------------------------------------------------------
        oclGlobalRange = (kp * (jp * ip))
        oclLocalRange = 0
        state_ptr(1) = ST_FEEDBF_MAP_35
        
        kp_ptr(1) = kp
        call oclWrite1DFloatArrayBuffer(kp_buf,kp_ptr_sz,kp_ptr)! Automatic conversion to array
        ...
        jp_ptr(1) = jp
        call oclWrite1DFloatArrayBuffer(jp_buf,jp_ptr_sz,jp_ptr)! Automatic conversion to array
        ip_ptr(1) = ip
        call oclWrite1DFloatArrayBuffer(ip_buf,ip_ptr_sz,ip_ptr)! Automatic conversion to array
